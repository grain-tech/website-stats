...
(function() {
  ...
  gsiScript.onload = function() {
    if (!CLIENT_ID) {
      ...
      if (gate) gate.remove();
      if (typeof window.grainOnAuth === 'function') window.grainOnAuth(null);
      return;
    }
  ...
  async function handleTokenResponse(response) {
    ...
    if (!getSession()) {
      try {
        ...
        setSession(info.email, info.name, info.picture);
        document.documentElement.classList.add('authed');
        window.grainUser = { email: info.email, name: info.name, picture: info.picture };
        const gate = document.getElementById('authGate');
        if (gate) gate.remove();
      } catch(err) {
    ...
    if (typeof window.grainOnAuth === 'function') window.grainOnAuth(response.access_token);
  }
